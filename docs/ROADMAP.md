...
Current release: iOS v1.0.4 (build 13), Expo SDK 56 / React Native 0.85
...
- Local-first income/expense logging on SQLite with soft deletes, migrations, and a free-tier record limit (currently 500 local records)
- Statistics and diagram screens (time filters, sorting, daily grouping, net totals; diagram with pie/treemap/bar/line modes, tap-to-detail category breakdowns, and custom date ranges)
- CSV export and CSV import
- 16 languages (en, zh-Hant, es, fr, de, ja, hi, pt, ru, id, ko, it, tr, vi, th, pl) with device-locale detection and customizable glossary terms
...
- iOS 1.0.4 live via local Xcode release flow (`RELEASE_IOS.md`); CI runs lint + typecheck + 63 unit tests
...
- Android build exists but has not shipped; in-app voice input exists only on web (iOS system-keyboard dictation ships)
...
- **Android launch**: the Gradle project and EAS profiles exist; needs device QA (auth, IAP, dictation permissions), Play Store products in RevenueCat, and store listing. Store-listing preview videos can be generated with `captures/preview-kit/` (Maestro → Simulator, 16 languages)
- **Native in-app voice input** on iOS/Android (iOS keyboard dictation already ships; the in-app speech API is still web-only Web Speech), if voice remains a product bet
...
- **Docs hygiene** — `CURRENT_CAVEATS_AND_NEXT_PERIOD.md` (originally 2026-04-27) has been refreshed for Google sign-in (live) and iOS dictation; keep folding status updates into this roadmap as the primary source of truth going forward
